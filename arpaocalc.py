...
class Ship:
    def __init__(self, position, speed, heading):
        ...
        if isinstance(heading, Number) == False:
            raise NameError('Your ship instance heading is not a float number')
...
def get_gmap_url(pointA, objectA_speed, vectorA_angle, pointB, objectB_speed, vectorB_angle, *args, **kwargs):
    ...
    gmap_url='https://maps.google.com/maps/api/staticmap?size=600x600&maptype=hybrid'
    ...
    if lonAcpa is not None and latAcpa is not None:
        marker_A_cpa = '&markers=color:green%7Clabel:1%7C' + str(latAcpa) + ',' + str(lonAcpa)
        gmap_url += marker_A_cpa

    if lonBcpa is not None and latBcpa is not None:
    ...
    if relvect is not None and relspeed is not None:
